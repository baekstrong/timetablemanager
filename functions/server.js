...
// ============================================
// CoolSMS 문자 발송 엔드포인트 (로컬 개발용)
// ============================================

const COOLSMS_API_URL = 'https://api.coolsms.co.kr';

function generateCoolSmsAuthHeaders() {
  const apiKey = process.env.COOLSMS_API_KEY;
  const apiSecret = process.env.COOLSMS_API_SECRET;

  if (!apiKey || !apiSecret) {
    throw new Error('CoolSMS API 인증 정보가 설정되지 않았습니다. (.env에 COOLSMS_API_KEY, COOLSMS_API_SECRET 추가)');
  }
...
async function sendCoolSMS(to, text, scheduledDate = null) {
  const from = process.env.COOLSMS_SENDER_PHONE;
  if (!from) throw new Error('COOLSMS_SENDER_PHONE이 설정되지 않았습니다.');
  ...
  const headers = generateCoolSmsAuthHeaders();
...
app.post('/sms/send', async (req, res) => {
  try {
    ...
    const result = await sendCoolSMS(to, text, scheduledDate);
...
app.post('/sms/send-batch', async (req, res) => {
  try {
    ...
    for (const msg of messages) {
      try {
        const result = await sendCoolSMS(msg.to, msg.text, msg.scheduledDate);
...
app.listen(PORT, () => {
  ...
  console.log(`📱 CoolSMS: ${process.env.COOLSMS_API_KEY ? '설정됨' : '미설정 (.env에 COOLSMS_* 추가 필요)'}`);
  console.log('');
