    ...
    return (
        <div className="makeup-modal-overlay" onClick={onClose}>
            <div className="makeup-modal" onClick={(e) => e.stopPropagation()} style={{ maxHeight: '80vh', overflowY: 'auto' }}>
                ...
                <p style={{ fontSize: '0.85rem', color: 'rgba(0,0,0,0.6)', margin: '4px 0 12px' }}>
                    {isDirectTransfer
                ...
                {!isDirectTransfer && (() => {
                    ...
                    return (
                        <div style={{ marginBottom: '12px', padding: '8px 12px', borderRadius: '6px', backgroundColor: '#EDBC401A', border: '1px solid #EDBC404D' }}>
                            <div style={{ fontSize: '0.85rem', fontWeight: 'bold', color: '#9a7a12', marginBottom: '4px' }}>현재 대기 ({existingWaiters.length}명)</div>
                            {existingWaiters.map(w => (
                                <div key={w.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', fontSize: '0.85rem', padding: '2px 0' }}>
                                    ...
                                    <button onClick={() => onWaitlistCancel(w.id)} style={{ fontSize: '0.75rem', padding: '2px 6px', border: '1px solid #EDBC404D', borderRadius: '4px', backgroundColor: 'transparent', color: '#9a7a12', cursor: 'pointer' }}>취소</button>
                                </div>
                            ))}
                        </div>
                    );
                ...
                <div className="makeup-modal-content">
                    ...
                    {waitlistStudentSearch && !waitlistStudentName && (
                        <div style={{ maxHeight: '120px', overflowY: 'auto', border: '1px solid #EFEFF0', borderRadius: '6px', marginBottom: '8px' }}>
                            {(() => {
                                    ...
                                    return parsed.map((schedule, index) => {
                                        ...
                                        return (
                                            <div key={index}
                                                ...
                                                style={isDisabled ? { opacity: 0.5, cursor: 'not-allowed', backgroundColor: '#F7F7F8' } : {}}
                                                onClick={() => {
                                            ...
                                            >
                                                ...
                                                {alreadyWaiting && <span style={{ fontSize: '0.8em', color: '#9a7a12', marginLeft: '8px' }}>이미 대기 중</span>}
                                            </div>
                                        );
                                    });
                                })()}
