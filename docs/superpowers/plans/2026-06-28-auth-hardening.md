...
## ⚠️ 실행 전 발견된 락아웃 벡터 (2026-06-28 스코핑) — 다음 세션에서 반드시 반영

Phase B를 실행하기 전 실제 코드를 확인한 결과, 단순 "비인증 차단" 규칙으로는 **앱·신규신청 funnel이 락아웃**된다. 아래를 모두 카브아웃해야 함:

1. **users 평문 비번 + 티어/학년 뱃지 broad read 얽힘** — users 문서에 아직 `password`(평문) 필드가 있고, `getTierMap`/`getGradeMap`(`firebaseService.js:1838,1956`)이 **전체 users를 `getDocs`로 읽음(수강생도 — 게시판 뱃지)**. users를 self/coach로 막으면 뱃지가 깨지고, 열면 평문 비번이 인증된 수강생에게 노출. → **Phase C(평문 제거)를 Phase B와 함께(또는 먼저) 해야 users broad read가 안전.**
2. **비로그인 신규등록 funnel** — `?register=true` → `NewStudentRegistration`이 `getDisabledClasses`/`getEntranceClasses`/`getFAQs`/`getNewStudentRegistrations`/`createNewStudentRegistration`/`updateNewStudentRegistration` 사용. → 비인증 **read**: `disabledClasses`·`entranceClasses`·`registrationFAQ`·`newStudentRegistrations`, 비인증 **create·update**: `newStudentRegistrations`. 카브아웃 없으면 **신규 신청 funnel 락아웃**.
3. **컨텍스트 마운트 시 holidays 읽기** — `GoogleSheetsProvider`가 `AppContent` 전체(로그인 화면 포함)를 감싸고(`App.jsx:466-470`), 마운트 `useEffect`(`GoogleSheetsContext.jsx:47`)에서 `fetchHolidays()`로 Firestore `holidays`를 **로그인 전에** 읽음. 막으면 `Promise.all` reject → `setError('초기화 실패')` → **앱이 안 뜸**. → `holidays` 공개 read 허용(또는 fetchHolidays를 로그인 후로 이동).
4. **로그인 폴백의 비인증 users read** — `Login.jsx`/훈련일지 `auth.js` 폴백이 비인증 상태로 `users/{name}` 읽음. 규칙 잠그면 폴백은 죽지만 serverLogin·세션공유가 정상이라 실사용엔 무방(폴백은 Phase C에서 제거).

## 보수 규칙 방향 (이 카브아웃 반영)
- `userSecrets`: **deny all**(클라). ⚠️ recursive `/{document=**}`로 broad 허용하면 OR 시맨틱상 userSecrets도 허용돼버림 → **최상위 `match /{collection}/{docId}`에서 special 컬렉션 제외**하는 구조로.
- **공개 read**: `holidays`, `disabledClasses`, `entranceClasses`, `registrationFAQ`.
- `newStudentRegistrations`: 공개 **read+create+update**(기존 동작 보존). PII 공개 노출은 기존과 동일 — 추후 개선 과제.
- 그 외 전부: `request.auth != null`.
- 중첩 깊이: **최상위 + `posts/{id}/comments` 2단계뿐**(확인됨). 3단계 없음.

## 실행 절차 (다음 세션)
1. Phase C(평문 제거)를 함께 묶기: users.password drop 스크립트 + 신규생성(`StudentRegistrationModal`/`CoachNewStudents`)·폴백에서 평문 중단 + `auth.cjs` checkPassword 평문 폴백 제거 + 본인 비번변경 서버 엔드포인트(C1). → auth.cjs는 `scripts/build-auth-bundle.cjs`로 재빌드 후 netlify-deploy 폴더 드롭.
2. `firestore.rules` 작성(위 카브아웃 반영).
3. **Firebase 에뮬레이터(`@firebase/rules-unit-testing`)로 본인/코치/비로그인 시나리오 테스트** — 배포 전 필수.
4. `traininglogforclients`에 배포(Firebase Console 규칙 붙여넣기 또는 `firebase deploy --only firestore:rules --project traininglogforclients`).
5. 배포 후: 로그인·게시판(뱃지)·시간표·훈련일지·신규등록 funnel 전부 동작 확인. 깨지면 즉시 규칙 롤백(개방).

---
### (이하 원래 B1/B2 태스크 — 위 발견사항으로 보강 필요)
