...
export async function saveNoticeSettings() {
    ...
    const isVisible = document.getElementById('noticeIsVisible').checked;
    ...
    if (!startDate || !endDate) {
        alert('시작 날짜와 종료 날짜를 모두 설정해주세요.');
        return;
    }

    try {
        await db.collection('notices').doc('shared').set({
            content,
            startDate,
            endDate,
            isVisible,
...
export async function loadNoticeSettings() {
    try {
        ...
        if (doc.exists) {
            ...
            document.getElementById('noticeIsVisible').checked = data.isVisible || false;
        }
    } catch (error) {
