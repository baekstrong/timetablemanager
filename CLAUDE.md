...
2026-09-25 사용자 결정: 이 프로젝트는 수정·검증 완료 후 별도 재승인 없이 자동 커밋·푸시한다. main 푸시에 따른 GitHub Pages 배포 결과까지 확인한다. 과거 로컬 확인 대기·자동 커밋 금지 기록은 이력이며, 특정 작업을 보류하라는 새로운 사용자 요청이 있을 때만 대기한다.

이번 작업 파일만 명시적으로 stage하고, 다른 작업자의 변경·비밀값·로컬 검토 산출물을 함께 커밋하지 않는다.
```bash
# 이번 작업 파일을 명시적으로 stage한 뒤 실행
...
### 주요 토큰
- 게시판 작성·수정 모달은 body 포털·z-index 2000으로 하단 내비게이션 위에 표시한다. 최대 너비 860px, 본문 높이 160~400px(화면 높이의 32%). 모바일 안전 영역을 제외한 동적 화면 높이 안에서 본문만 스크롤하고 제목·작성 버튼은 고정한다. 열릴 때 body를 fixed로 잠가 iOS 배경 스크롤을 막고, 닫힐 때 기존 인라인 스타일과 스크롤 위치를 복원한다.
...
| `ranking` | Ranking | 랭킹·내 PR·성장 그래프 (수강생은 내 수업의 성장/이달의 PR 카드, 코치는 Dashboard 카드로 진입) |
...
- 코치가 `재개`를 누르면 `ResumeStudentModal`이 열리고 재시작일·주횟수(주1~5회)·새 요일/교시를 입력받는다. 정지 전 주횟수를 기본 선택하고, 같은 요일에는 한 교시만 선택할 수 있다. 선택 슬롯 수가 주횟수와 정확히 같아야 종료일 계산·저장이 가능하며 서비스에서도 이를 재검증한다.
- 모달은 `getPausedStudentResumeInfo()`로 정지된 모든 등록의 남은 횟수를 한 번에 읽고, `calculatePausedStudentResumePlan()`으로 선택 시간표·공휴일을 반영한 실제 시작일과 최종 종료일을 즉시 보여준다. 재시작일이 수업일이 아니면 그 이후 첫 선택 수업일을 실제 시작일로 사용한다. 자동 종료일은 코치가 직접 수정할 수 있고, 재시작일·주횟수·시간표를 다시 바꾸면 수동값을 버리고 새 자동값으로 돌아간다. 정지 등록이 여러 건이면 직접 지정한 날짜는 마지막 등록의 종료일에만 적용하며 그 등록의 시작일보다 빠를 수 없다.
...
### 일시정지 잔여 합계·차감 (2026-09-09)

- 코치 수강생 목록은 정지 등록을 합산해 `총 N회 (등록 M건)`을 표시한다. 합계는 `_pausedTotal`/`_pausedCount`에 두며 원본 H를 덮어쓰지 않는다. 0회 등록은 정지 목록 및 재개 대상에서 제외한다.
- 최근 창(-6~+2개월) 밖의 장기 정지도 누락되지 않도록 오래된 시트의 B:H만 추가 배치 조회한다. 최근 창 조회와 병렬이며, 오래된 행은 양수 잔여 정지만 목록에 추가한다. 일반 등록·결제·연락처 전체 조회로 확대하지 않는다.
- `AdjustPausedSessionsModal`은 최종 잔여(0 이상, 현재보다 작은 정수)와 사유를 입력받아 원래 수강 시작일 순서로 차감 미리보기를 보여준다. 결제월 순서가 아니다.
- `adjustPausedStudentSessions`는 저장 직전 전 시트의 정지 행·메모 스냅샷을 비교하고 달라졌으면 저장을 거부한다. 변경 행의 H/E만 batchUpdate 1회로 반영한다. E에는 한국 날짜·등록별 전후·전체 전후·사유를 기록하고, 0회 행은 정지 태그를 제거해 구버전에서도 재개되지 않게 한다. 등록·결제 행, 실제 출석, 홀딩 기록은 보존한다. 정지 차감은 Sheets 작업이며 Firestore 홀딩/결석 이중 쓰기 대상이 아니다.
- 저장 오류 시 응답 유실 가능성이 있으므로 자동 재저장하지 않고 최신 정보 재조회를 요구한다. 사전 비교는 Sheets의 원자적 compare-and-swap이 아니므로 동시 운영자 수정까지 잠그는 기능은 아니다.
- 검증: `src/services/pausedAdjustment.test.js`(분산 차감, 0회 소진·재개 제외, 전량 소진, 오래된 정지 합계, 변경 충돌·입력 거부), 로컬 브라우저의 모의 시트로 모바일 미리보기·저장 검증.
...
- 신청 데드라인: 홀딩 시작일 수업 시작 **2시간** 전
- 취소 데드라인: 홀딩 시작일 수업 시작 **1시간** 전 (보강일 포함, `getClassPeriod` 사용)
...
- `getUnappliedHoldings()`(status active + `sheetsApplied==false`)를 수강생 관리 상단 경고 배너가 읽는다. 마운트 시 1회, 코치 화면 전용, 화면을 막지 않는다(실측 35ms·0건). `sheetsApplied` 단일 필드 인덱스에는 그 필드를 가진 문서만 들어가므로 **옛 문서가 아무리 쌓여도 스캔 대상이 안 늘어난다.**
- 배너 각 줄의 **`확인함` 버튼**(`markHoldingSheetsApplied`로 `true` 마크)을 빼지 말 것. 시트 쓰기는 성공했는데 마크만 실패하면(마지막 write 직전 네트워크 끊김) 고칠 게 없는 경고가 영영 남고, 그런 게 쌓이면 코치가 배너 자체를 무시하게 돼 진짜 유실을 놓친다. 오클릭으로 진짜 유실이 숨지 않도록 `confirm`으로 한 번 막는다.
...
- 신청 데드라인: 원본 수업과 보강 대상 수업 모두 시작 **2시간** 전까지
- 취소 데드라인: 보강 수업 시작 **1시간** 전까지
...
만석 슬롯 클릭 시 대기 신청 모달(`MakeupModal` 재사용)에서 원래 수업을 선택해 `makeupWaitlists` 컬렉션에 등록한다. 자리 발생 트리거(홀딩 신청/결석 신청/보강 취소·거절 + 코치 시간표 로드 백스톱)가 실행되면 대기 1순위에게 자리 안내 SMS를 발송하고 status를 `notified`로 변경한다. 수강생은 시간표의 '보강승인중' 칩을 클릭해 1시간(수업 시작이 더 가까우면 그때까지) 내에 수락 또는 거절할 수 있다. 수락 시 정식 보강(`makeupRequests`)으로 확정되고 종료일이 재계산된다. 거절하거나 시간 초과로 만료되면 다음 순번에게 동일하게 안내한다. 활성 대기(waiting/notified)는 신청 단계에서 주간 보강 약속 수에 포함하며, 수락할 때 자신의 대기를 제외한 사용량으로 다시 검증한다. 취소된 보강 이력은 횟수가 복구되지 않고 만료된 대기는 활성 약속에서 제외한다(`makeupQuota.js`, `StudentSchedule.jsx`).
...
- **저장/갱신(본인)**: App의 `useStudentGrowth`가 `refreshStudentTier({userName, deferSeen:true})`·`refreshStudentXP({userName, gender, deferSeen:true})`를 호출한다. 학생 접속 시 지난달 티어·저장된 누적 훈련량 기반 학년을 갱신하며, 조회만으로 확인 상태를 소비하지 않는다. 같은 달 티어 재계산은 no-op. 첫 계산은 인트로 안내. 기존 호출의 기본 동작은 유지한다.
- **일괄 백필(코치)**: `backfillTiersForMonth()` — 코치 진입 시 호출되지만 `studentMeta/tierBackfill` 문서(`{month}`)로 **그 달 첫 1회만** 실제 실행(그 외엔 문서 1회 읽고 종료). 지난달 `records`를 날짜 범위로, `freeWorkoutAttendance`를 통째로 **컬렉션당 1회** 읽어 이름별 날짜 집합으로 그룹핑 → 이번 달 미계산 학생만 batch write. 스캔 **전에** 잠금을 먼저 써서 코치 2명·탭 2개 동시 진입 시 중복 스캔을 막는다. ⚠️ 이게 없으면 메인 앱을 안 여는 학생(훈련일지만 쓰는 학생)의 뱃지가 지난달 값에 영구히 멈춘다 — 2026-08에 실제로 62명 중 32명이 6월 기준 메달을 달고 있었다. 백필 대상은 `tierIntroPending=true`로 표시 → 그 학생이 다음 내 수업 진입 시 저장된 `prevTier`와 비교한 통합 팝업을 보여주며, 확인 버튼에서 해당 월·티어가 일치할 때만 플래그를 해제한다.
- **뱃지**: `TierBadge.jsx`. 게시판은 `getTierMap()`(이름→티어, 5분 캐시)을 Dashboard에서 읽어 PostList/PostDetail/CommentItem에 prop으로 전달. 코치는 뱃지 없음.
- **팝업**: `StudentGrowthModal.jsx` — 내 수업에서 티어·학년 변경을 하나로 안내한다. App 수명에 상태를 유지하며 게시판에서 확인 이벤트를 소비하지 않는다. `studentGrowthService`의 트랜잭션으로 실제 확인한 월/티어와 최고 학년(`gradeSeen`)만 갱신한다. 나중에/닫기는 미확인 유지. 코치 빙의·실데이터 조회 전용 화면은 계산·확인 쓰기를 하지 않는다.
- **훈련일지 학년 팝업**: 저장 직후 학년 상승 안내는 유지하며 확인 성공 시에만 `gradeSeen`을 올린다. 훈련일지에서 확인한 학년은 메인 내 수업에서 다시 안내하지 않는다. 동시·지연 응답으로 확인한 최고 학년을 낮추지 않는다.
...
13. **시트 원격 호출은 반복문 안에 넣지 말 것 (이 저장소에서 가장 자주 재발한 성능 버그).** 왕복 1회가 프로덕션 실측 **≈0.73초**(콜드스타트 3.7초)라 루프에 들어가면 그대로 초 단위로 쌓인다. 페이로드는 15개 시트 전부 합쳐 89KB로 작아서 **병목은 언제나 데이터 양이 아니라 왕복 횟수**다.
    - 읽기 여러 개 → `batchReadSheetData(ranges)` 1회. 쓰기 여러 개 → `batchUpdateSheet(updates)` 1회. **`batchUpdate`의 range는 시트명을 포함하므로 여러 시트를 한 요청에 담을 수 있다** — 시트마다 따로 쓸 이유가 없다.
    - 색칠은 `highlightAsync`/`formatCellsAsync`로 **던지고 기다리지 않는다**. 데이터 쓰기 뒤의 장식이라 await 하면 왕복 1회만큼 사용자를 더 붙잡는다.
    - 함수를 학생 수만큼 부르는 경로(휴일 일괄 변경, 입학반 일괄 배정)는 **collector/캐시를 넘겨 쓰기를 모으고 같은 시트를 다시 읽지 않게** 한다. `adjustNextRegistration(..., collector)`, `applyNewScheduleToNextRegistration(..., collector)`, `findApprovedStudentRow(reg, sheetCache)`, `applyEntranceAssignment(..., batch)`가 그 패턴이다.
    - **다음 등록(미리 등록) 조정을 따로 쓰지 말 것.** `collectNextRegistrationUpdates(found, newEndDate, holidays)`로 모아 본 업데이트와 한 요청에 실는다 — 홀딩·홀딩취소·코치홀딩·휴일보강·시간표이동·휴일delta 6곳이 전부 이 규약을 쓴다. 따로 쓰면 미리등록이 있는 수강생만 왕복이 1회씩 더 늘고, 본 등록만 쓰이고 다음 등록이 실패하는 비원자적 상태도 생긴다.
    - **재등록 검색은 `findStudentAcrossSheets(name, { requireActive: false })`.** 기본값(`true`)은 윈도우에서 찾았어도 **오늘 활성인** 등록이 없으면 전 시트를 다시 읽는다 — 장기 등록 행이 결제한 달 시트에 남아 윈도우 밖일 수 있어 학생 로그인·대시보드에는 필요한 안전장치다. 하지만 **재등록에는 해롭다**: 재등록은 직전 수강이 끝나고 며칠 뒤에 이뤄져(실측 중앙값 5일, 75%가 8일 이내) '오늘 활성'이 거의 항상 false인데, 직전 등록 행 자체는 최근 시트에 그대로 있다. 실제 재등록 421건을 재보니 **75.8%가 윈도우 안에 행이 있어 폴백이 불필요**했고 정말 필요한 건 10.7%(오래 쉬었다 돌아온 경우)뿐이었다. 실측 2.6초 → 0.85초. ⚠️ **"재등록 대상은 수강이 끝났으니 윈도우에 없다"는 추정은 틀렸다** — 끝난 건 등록이지 행이 아니다.
    - **본인 등록 조회(학생 로그인·빙의·대시보드)도 `requireActive: false`** — `App.jsx`의 `STUDENT_LOOKUP` 상수와 `Dashboard`의 종료일 확인. 여기도 안전장치 역할을 못 하고 있었다: 실제 학생 계정 69개를 대조하니 폴백이 발동하는 37명 중 **25명은 윈도우에 행이 0건**이라 `allMatches.length === 0`으로 어차피 폴백을 타고(장기등록 안전장치는 그대로 살아 있다), 나머지 **12명은 폴백을 껐을 때 선택되는 등록이 한 명도 달라지지 않았다**. 즉 그 12명은 배치 읽기 1회(실측 ≈790ms)를 헛쓰고 있었다.
    - **본인 등록 조회에서 현재월을 먼저 따로 읽지 말 것.** 예전 `App.jsx`는 `getStudentByName`(현재월 1장)을 `await`한 뒤 `findStudentAcrossSheets`(±2개월)를 불렀는데, 앞의 시트는 뒤 윈도우의 **부분집합**이라 내용이 중복이면서 왕복만 1회 늘었다(실측 1161ms → 614ms). "빨리 뭐라도 그리자"는 의도였지만 그때 그려지던 게 아래의 목 데이터였다. `getStudentByName`은 이때 마지막 사용처가 사라져 컨텍스트에서 제거했다.
    - **읽기 전에 걸러라.** `processHolidayMakeupEndDate`는 공휴일 여부를 시트를 읽기 **전에** 판정한다 — 순수 계산이라 시트가 필요 없다. 실측 보강 641건 중 원수업이 공휴일인 건 7건(1.1%)뿐이라 **98.9%가 시트를 아예 안 읽는다.** 이 사전 필터를 지우면 보강 신청·대기수락마다 전 시트 batchGet이 되살아난다.
    - **범위도 필요한 만큼만.** 등록 경로의 행 탐색은 번호·이름만 쓰므로 `A:B`다. `A:R`로 되돌리면 페이로드가 9배가 된다(왕복 수는 그대로).
    - **서로 결과를 안 쓰는 호출은 `Promise.all`로.** 다른 컬렉션 조회, 완료 후 리로드 2~3개, 날짜별 독립 문서 생성 등은 직렬로 기다릴 이유가 없다. 단 **진짜 의존은 그대로 둘 것** — 홀딩의 Firestore→시트 순서와 롤백, 시트 write 후 refresh(read-after-write), 푸시 실패 시 SMS 폴백, 입학반 인원 read-modify-write(병렬화하면 lost update로 정원이 어긋난다), `users` 문서 생성 → 비밀번호 설정.
    - **실패해도 무방한 부수작업은 await 하지 말 것.** 보강 대기 알림(`onSeatsFreedForDates`)은 이미 완료 alert를 띄운 뒤라 기다리면 대기자 SMS/푸시가 끝날 때까지 차단 화면이 남는다. 게시판 푸시·`highlightAsync`·`formatCellsAsync`가 같은 규약이다.
    - 회귀 방지: `src/services/sheetRoundTrips.test.js`가 `fetch`를 가로채 **왕복 횟수 자체를 검증**한다(수강 종료·일시정지·홀딩·보강·정보수정 등). 반복문 안 호출이나 읽기-후-판정으로 되돌리면 이 테스트가 깨진다. 지우지 말 것.
14. **`findStudentInSheets`는 시트를 `batchReadSheetData`로 한 번에 읽는다 — 시트별 순차 읽기로 되돌리지 말 것.** 홀딩·결석·보강·시간표변경 등 쓰기 경로 6곳이 전부 이 함수를 거치는데, 탭마다 왕복하면 탭 수(현재 15개)만큼 지연이 쌓인다. 실측 7.5초 → 0.6초(**12.6배**, 구글 직결 기준이며 앱은 Netlify를 한 번 더 경유하므로 실제 차이는 더 크다). 이 대기 중에 수강생이 앱을 닫아 신청이 시트에 반영되지 않는 사고가 있었다. 요청 범위는 실제 존재하는 탭만 넣을 것 — 없는 범위가 하나라도 섞이면 batchGet 전체가 400이다. 개별 읽기 폴백은 일부러 없다(활성 등록이 든 시트만 실패하면 엉뚱한 행에 쓴다)
15. **훈련일지 `loadPinnedMemosForSelectedStudents`의 캐시 가드를 없애지 말 것** — `renderPinnedMemosForCoach`가 `loadAllRecords`의 `onSnapshot` 콜백 안에서도 불리므로, 캐시가 없으면 학생이 기록을 저장할 때마다 선택 인원×2 문서를 다시 읽는다(7명이면 스냅샷 1회당 14 read). 문서를 직접 고친 쓰기 경로는 캐시를 같이 갱신하거나 `renderPinnedMemosForCoach(이름)`으로 그 학생만 재조회시킨다
16. **아직 안 온 시트 데이터 자리에 "그럴싸한 기본값"을 넣지 말 것.** `StudentInfo`는 `studentData`(구글 시트)가 없을 때 목 데이터(시작·종료일, 주2회, 남은 8회, 가짜 출석 2건)를 그렸는데, 로딩 오버레이가 **Firebase 3건만** 기다리고 시트는 안 기다려서 그 가짜가 수강생 화면에 1~2초 진짜처럼 떴다가 바뀌었다. 시트에 행이 없는 계정(3명 확인)은 **영구히** 남의 수강 기간을 봤다. 지금은 ①`App.jsx`가 `isLoading={isStudentDataLoading}`을 넘기고 ②`stillLoading = loading || isLoading`으로 시트까지 덮으며 ③끝내 못 받으면 "수강 정보를 찾을 수 없습니다"를 띄운다. 회귀 테스트 `src/components/studentInfoLoadState.test.js`가 배선·금지 패턴을 소스에서 직접 확인한다(이 저장소엔 jsdom이 없어 컴포넌트를 못 띄운다)
17. **훈련일지 코치 진입: 조회 범위를 다시 넓히지 말 것.** 화면에 그리는 건 **학생당 하루치**(`renderCoachSessionBlock`)인데 `renderCoachSessionView`가 90일치를 받던 시절, 명단 7명 기준 **records 560건·2.85MB**가 매 진입마다 내려왔다(수강생 진입은 같은 시점 ≈100건·250KB — **7배 문서, 12배 데이터**). 지금은 `SESSION_DAYS = 14`(104건·467KB)로 받고, 그 이전은 날짜 드롭다운의 `⋯ 더 오래된 기록 불러오기`(`SESSION_DAYS_MORE = 180`)로 **그 학생만** 다시 받는다.
    - **14일 안에 기록이 없는 수강생 자리에도 '이전 기록 불러오기' 버튼을 남길 것.** 없으면 오래 쉰 수강생이 '기록 없음'으로 끝나 지난 기록을 볼 길이 막힌다.
    - **진입 조회는 `prefetchEntryData()`가 users 조회와 나란히 던진다.** 예전엔 `users.get()` → `schedules` → `roster` → `tags` → `records`가 직렬 5단이었다. `applyCurrentClassRoster({ prefetched: true })`는 이 캐시를 그대로 쓰므로 **캐시 비우기를 그 안으로 되돌리지 말 것**(프리페치가 통째로 버려진다). 🔄 새로고침 경로는 `prefetched:false`로 같은 6건을 `Promise.all`로 던진다.
    - **`initApp`은 `paintShell()`로 뼈대를 먼저 그리고 나서 `autoLogin`을 기다린다.** 예전엔 `await Auth.autoLogin()`(Auth 복원 최대 2.5초 + 실패 시 서버 재인증)이 끝날 때까지 `#app`이 비어 있어 앱 간 이동이 백지로 보였다. `paintShell`에서 Firestore를 건드리면 안 된다 — 규칙이 `signedIn`을 요구해 인증 전 조회는 permission-denied로 죽는다.
    - ⚠️ **`paintShell` 직후 `state.currentUser`/`isCoach`를 반드시 다시 비울 것.** 로그인 판정은 예전 그대로 `state.currentUser` 하나여야 한다. 처음엔 `autoLogin`이 성공 여부를 **반환**하게 고쳤는데, 배포 직후 **코치가 로그인 화면으로 튕기고 수강생 목록이 '로딩중'에서 멈췄다** — 훈련일지 JS는 **해시 없는 ES 모듈**이라 브라우저가 `main.js`와 `auth.js`를 **따로** 캐시 갱신하고, '새 main + 옛 auth' 조합에서 옛 `autoLogin`이 `undefined`를 돌려주자 새 main이 세션을 지워버린 것이다. **훈련일지 모듈 사이에 새 계약(반환값·인자·시그니처)을 만들지 말 것** — 한쪽만 갱신된 조합이 실제로 사용자에게 뜬다. 불가피하면 양쪽이 옛 버전과도 동작하게 짤 것.
    - **부팅 스플래시는 `#app` 안의 정적 마크업 + 인라인 `<style>`이다**(`index.html`). `#app`이 비어 있던 시절엔 firebase CDN 3개와 ES 모듈 16개가 평가돼 `paintShell()`이 돌기 전까지 회색 백지였다. JS·이미지·타이머를 쓰지 않으므로 모듈 평가 전 첫 프레임에 뜨고, `#app.innerHTML`을 덮어쓰면 **저절로 사라진다** — 별도 숨김 로직을 만들지 말 것(`#app` 밖으로 옮기면 그게 필요해진다). 0.25초 지연 fade-in(빠른 날 깜빡임 방지)과 8초 뒤 '네트워크가 느린 것 같습니다' 안내도 전부 CSS 지연 애니메이션이다. 마크업과 스타일을 같은 파일에 두는 건 구버전 사파리가 CSS와 모듈을 따로 캐시하기 때문.
    - **명단은 네트워크보다 먼저 그린다.** 앱 전환(`window.location.href`)은 페이지 전체 재로드라 진입마다 TLS 3호스트 + Auth 토큰 + Firestore 핸드셰이크를 처음부터 맺는데, 각 층이 식는 주기가 달라(라디오 수십초 / 소켓 / ID 토큰 **1시간** / Netlify Lambda 수분) **첫 진입만 느리다**. 실측: `accounts:lookup` 256ms, `users` 71문서 26~60ms(웜), Netlify `auth/login` **콜드 3.58초**(웜 0.75초). 예전엔 그 두 왕복(autoLogin → `users.get()`)이 끝나야 명단이 처음 그려져 간헐적으로 초 단위였다.
        - `coach.js`가 이름 목록을 `localStorage.allStudentsCache`에 캐시하고 **`users.get()`을 기다리기 전에** `paintStudentList`로 그린다. 서버 결과는 달라졌을 때만 다시 그리고, 조회가 실패해도 이미 그린 명단을 지우지 않는다. iOS에서 `enablePersistence`의 IndexedDB 오픈까지 첫 쿼리가 큐에 걸리던 것도 같이 우회된다.
...
- 사용자 화면 확인 후 커밋·main 푸시를 명시적으로 승인했다. main 푸시는 GitHub Pages 프론트엔드 배포로 이어진다. Netlify 함수·Firestore 규칙의 별도 운영 배포는 아직 수행하지 않는다.
- 코치만 `today`를 기본 홈으로 사용한다. 하단 메뉴는 오늘/시간표/훈련일지/수강생/신규. 수강생 UI·메뉴·첫 페이지·홀딩/계약 흐름은 기존 상태를 유지한다.
- `src/features/today/`: 코치 상단 확인할 일, 왼쪽 오늘 교시, 오른쪽 선택 교시 참석자의 코치 전용 메모. 확인할 일이 0건이면 영역 전체를 숨긴다.
- `WeeklySchedule`의 `view` 속성으로 기존 데이터 경로를 재사용한다. 코치 메모는 기존 `coachNotes/notes`와 병합 저장한다. 재등록은 기존 등록 모달을 사용하며 하단 메뉴 위에 표시한다.
- `ux-preview.html`은 Vite 개발용 화면 검토 진입점으로 기본 배포 빌드에 포함하지 않는다. 수강생 목업은 과거 검토 기록이다. 코드·검증·남은 범위 정본은 `docs/ux-redesign-review.md`.

### 수강생 UX 검토 재개 (2026-09-24)

- 개발 전용 `ux-preview.html?role=student`에서만 `BottomNav`의 preview 옵션과 DEV 조건으로 제안 메뉴(홈/훈련일지/게시판/내 정보)를 표시한다. 운영 수강생 메뉴는 기존 그대로다.
- 화면 검토 및 적용 범위는 `docs/ux-redesign-review.md`를 따른다.

- `src/preview/StudentTimetablePreview.jsx`는 시간표 중심 수강생 홈 목업이다. 예시 상태에서만 보강/대기·홀딩/결석을 반영하며 운영 서비스를 호출하지 않는다.

### 수강생 UX 재검토 제안 (2026-09-24 후속)

- 사용자 재검토·제안 요청으로 `outputs/student-ux-review-2026-09-24/`에 검토 보고서와 대화용 화면 제안을 작성했다. 당시 내 일정 우선·보강 시간표 분리는 미채택 제안이었다. 이후 9/25 사용자가 전체 흐름을 수용했으며, 훈련일지 확정 뒤 실제 적용을 시작하기로 했다.
- 최신 검토 상태·기존 목업 이력은 `docs/ux-redesign-review.md`. 마감 시각·보강 대기 쿼터는 문서와 구현에 차이가 있어 향후 구현 전에 현행 정책과 일치시켜야 한다.

### 수강생 동작 목업 (2026-09-24)

- `student-ux-mockup.html` → `src/preview/student-class-main.jsx` → `StudentClassMockup.jsx`와 `studentClassMockupModel.js`, 스타일 `student-class-mockup.css`. 기존 `ux-preview.html`과 별도 진입점이다.
- 개발 환경에서만 렌더하고 운영 서비스·인증·영구 저장을 사용하지 않는다. 기본 배포 입력에 추가하지 않았다. 보강 당주 제한, 대기/수락, 연속 홀딩·결석, 메뉴 이동을 가상 데이터로 검토한다.
- 검토 주소: `http://127.0.0.1:5178/timetablemanager/student-ux-mockup.html`. 예시 날짜는9/24목13시, 휴일을 제외하지 않은 수금 일정. 최신 검증/캡처는 `outputs/student-ux-review-2026-09-24/mockup-notes.md`.

### 훈련일지 목업 구체화 (2026-09-25)

- 전체 흐름은 사용자가 수용했고 훈련일지 최종 확인 후 실제 앱 작업을 시작한다. 지금은 개발 목업만 보완했다.
- `TrainingLogMockup.jsx`·`training-log-mockup.css`는 같은 목업의 훈련일지 탭이다. `?page=training`으로 직접 진입한다. 지난 값과 오늘 입력·참조회차·종목별 초안을 분리하며 하단 탭 이동에도 초안을 유지한다. 메모리 상태뿐이므로 새로고침하면 초기화된다.
- 생산 코드/운영 데이터와 연결하지 않는다. 기존 훈련일지 단위·날짜·초안 보존 방침과 검증 정본은 `docs/ux-redesign-review.md` 및 `outputs/student-ux-review-2026-09-24/mockup-notes.md`.

### 운동 검색·디자인 범위 후속 (2026-09-25)

- `ExerciseSearchMockup.jsx`는 개발 목업 전용 종목 검색 combobox다. 타이핑 후보/최근선택/키보드 선택/IME 조합 보호를 제공하며 확정 종목과 미확정 검색어를 분리한다. `TrainingLogMockup.jsx`의24개 가상종목을 사용한다.
- 사용자 검색 요청을 반영했다. 기존 훈련일지에도 검색 자동완성이 있으므로 실제 적용에서 공용/개인 종목 검색·직접 추가를 보존한다. 목업 전용 select는 제거했다.
- 전체 흐름 수용과 새 외관 전체 교체는 다르다. 실제 적용은 기존 디자인·기능을 유지하는 방향이다. 목업 하단바·제목·훈련일지 카드/입력 형태를 기존에 가깝게 조정했으며 운영 파일 변경이나 배포는 없다.

### 월간 훈련일지 목업 (2026-09-25)

- `?page=training`의 첫화면은 `TrainingCalendarMockup.jsx`/`training-calendar-mockup.css` 월간달력이다. 오늘기록 버튼으로 기존 작성폼에 들어간다. 샘플자료/표현은 `trainingLogMockupData.js`에 분리한다.
- `TrainingLogMockup`의 달력조회와 작성날짜TODAY는 별개다. calendar/write를 hidden으로전환하며 종목별초안 유지, 저장기록에서고유운동일수를파생한다. 달력에서확인 버튼의key변경은달력선택만오늘로초기화하며작성상태는유지한다.
- 운영앱과 미연결인 개발용목업이다. 검증/캡처정본은 `outputs/student-ux-review-2026-09-24/mockup-notes.md`.

### 수강생 UX 실제 구현·배포 승인 (2026-09-25)

- **현재 승인: 사용자가 로컬 최종 확인 후 “좋아 배포가자”로 커밋·푸시·배포를 승인했다. 업데이트 공지도 승인했으며 문장마다 줄바꿈·문단 분리를 적용한다.** 아래 자동 Git/배포 금지 문구는 로컬 검토 당시 이력이다.
- 최초 개발 승인은 자동 커밋·푸시·배포 없이 로컬에서 먼저 확인하는 범위였다. 현재는 위 명시적 배포 승인이 우선한다.
- 학생 로그인 기본/빙의 시작/뒤로가기는 `schedule`(내 수업). `getLoginPage`가 기존 훈련일지 복귀 경로와 게시글 딥링크를 보존한다. 하단 네 메뉴는 내 수업·훈련일지·게시판·내 정보. 홀딩 하위 화면은 내 수업, 계약은 내 정보가 활성화된다. 코치 메뉴는 유지.
- `StudentSchedule`은 실제 학생에게 `StudentClassView`를 렌더. `studentClassModel`로 등록별 주간 일정/휴일/홀딩/결석/보강을 구성한다. 개인 주 이동은 별도 읽기이며 보강 목적지는 기존 이번 주 정책을 유지한다. 원수업→목적지→확인과 대기 제안/수락은 공용 `ReviewModal`을 사용하며 기존 서비스 처리에 연결한다.
- `HoldingManager`는 선택 날짜 맥락/접힌 안내/확인창, 공통 날짜·마감·기존 신청 재검증을 적용한다. 현재·이전·다음 등록의 요일·교시/주횟수를 날짜별 판정하며 취소 시 시작일을 `cancelHoldingInSheets`의 optional 마지막 인자로 전달해 해당 등록 행을 찾는다. 신청·취소/관련 새로고침 후 App의 학생 정보도 갱신한다. 기존 Sheets/Firestore 이중 쓰기를 유지한다. 본문의 낡은 마감 안내를 실제 코드(홀딩/보강 신청2시간·취소1시간)와 맞췄으며 정책 변경은 아니다.
- 훈련일지 `student-workspace.js`/`student-workspace-logic.js`: 월간 달력 첫 화면, 기록 고유일/피드백 구분, 조회일(`selectedDate`)과 작성일(`studentWriteDate`) 분리, 검색 자동완성, 최근3운동일 참고/명시적 채우기, 날짜·종목별 localStorage 초안. 이전 autosave는 작성일 확인 표시로 가져오고 저장 실패 입력은 원사용자 복구함에 보존한다. 오프라인 저장·XP/PR 기존 시점, 메모/통증/단위/1RM/도장/코치 경로 유지. Tailwind 정적 CSS 재생성 포함.
- 검토 진입점: `student-local-review.html`, `training-log-review.html`(DEV/localhost만). 실제 컴포넌트/모듈에 메모리 서비스, 분리 저장소·외부 API 차단을 적용한다. 기본 프로덕션 진입점/빌드 대상이 아니다. 실제 앱의 `/timetablemanager/`는 기존 운영 서비스 연결을 유지한다.
- 검토 정본/검사/캡처: `outputs/student-ux-local-2026-09-25/review.md`. 승인 상태는 `docs/ux-redesign-review.md`. 운영 쓰기·메시지 발송·커밋·푸시·배포는 실행하지 않는다.
- 최종 검증: 전체58파일/518테스트·빌드·TL CSS 빌드, 핵심 변경 ESLint 오류0(기존 App/StudentInfo 경고2),390/320px 실제 코드 메모리 검증·실제 Chrome 검토 화면 확인. 운영 쓰기·iOS 실기기는 미검증, 로컬 사용자 검토 대기.

### 주간 달력 상태 후속 (2026-09-25)

- `StudentClassView`는 출석 대상 정규/보강과 실제 자율운동 출석일을 파란색, 선택 날짜는 테두리로 구분한다. 지난 수업의 배경을 회색으로 덮지 않는다. 정규 출석 예정/지난 수업, 보강/보강이동/홀딩/결석/보강홀딩/보강결석/휴일 상태 유지, 하루 여러 상태는 전부 표시한다.
- `studentClassModel`의 자율운동 출석은 본인의 `freeWorkoutAttendance` 실제 기록만 반영하고 `roster:true` 고정 명단은 제외한다. 정규 수업 경과를 실제 출석으로 단정하지 않는다. 주 이동에도 기존 `getFreeWorkoutByDateRange`를 주입해 조회한다.
- 모델14테스트·변경 ESLint·빌드·390/320px 로컬 검증 통과. 자동 Git/배포 금지 유지, 검토 정본의 후속절 참조.

- 상태 칩 후속: 코치 `StudentTag` 내부 배지를 공용 `ScheduleStatusBadge`로 추출, 수강생 주간 달력/수업 상세에서도 기존 `TAG_STYLES`·`.status-badge`를 사용한다. 학생 범위에서만 크기·여백 조정. 보강 파랑/이동 앰버/결석·홀딩 붉은색. ESLint·빌드·390/320px 확인, 로컬 상태 유지.

### 실데이터 검토·공지 후속 (2026-09-25)

- `student-live-review.html`은 DEV/localhost에서 정상 Firebase Auth 세션으로 실제 등록/시간표를 조회한다. 코치만 학생을 검색/선택, 학생은 본인 고정. `useWeeklyData`/`useScheduleCore`/`StudentSchedule`의 `readOnly` 기본값은 false이며 이 진입점에서만 자동완료/신청/대기/취소/알림을 막는다. 전용 네트워크 가드도 읽기·정상 로그인만 허용한다. 운영 데이터 쓰기로 검증하지 않는다.
- `NoticeTicker`는 실제 내 수업 최상단의 한 줄 공지다. `posts.category=notice && pinned === true`인 미삭제 공지만 표시하며 복수이면 6초 순환, 직접 넘기기/멈춤, 확인 전 N. `noticeService`는 `users/{name}.noticeReads`에 공지별 revision을 트랜잭션으로 기록한다. 기존 `PostDetail`의 최신 사용자/글 상세 로드 뒤만 읽음 처리하며 삭제글/늦은 응답은 제외한다. 게시판 진입/순환은 확인이 아니다.
- 실제 읽기 전용 미리보기에서 공지 상세는 조회만 하고 확인 상태를 쓰지 않는다. 예시 검토에서만 분리된 localStorage로 N 해제를 검증한다. 전체61파일/539테스트·빌드·ESLint·모바일 검증 통과. 사용자 코치 로그인 후 지정 수강생의 실제 수업·보강 시간표·공지 조회 확인, 고정 공지 필터 10테스트·ESLint·빌드 통과. 커밋·푸시·배포 금지 유지. 검토 정본 후속절 참조.

- 하단 메뉴 후속(2026-09-25): 실데이터 검토에 공용 BottomNav 네 메뉴 연결. 실제 수업/게시판/내 정보·월별 훈련기록을 읽기 전용으로 확인하며 훈련 작성 동작은 기존 격리 예시 진입점을 사용한다. 학생 메뉴 최대700px·56px 터치·안전영역 여백, 서브앱 시각/선택 접근성 통일. 실제 Chrome 390px·예시320px 왕복, 전체62파일/541테스트·빌드 확인. 사용자 최종 확인 후 배포 결정, 커밋·푸시·배포 금지 유지.

- 최종 검토 후속(2026-09-25): 실데이터 holding route/날짜 문맥을 LiveHoldingReview→HoldingManager(readOnly 기본false)로 연결. 날짜선택·확인모달 가능/최종 신청·취소 차단. 학생 주간 탐색은 제거하고 일요일0시·화면복귀 때 현재 주를 갱신, 주변경시 선택·신청모달 초기화. 주간 날짜16px. 보강 grid는 StudentMakeupSlot 공용 칩으로 본인 상태 우선·자리/마감 보조, disabled색 유지. 전체63파일/551테스트·빌드·실제 Chrome/320px 검증, 운영 쓰기와 커밋·푸시·배포 없음.

### 수강생 UX 배포 최종 점검 (2026-09-25)

- 현재 main과 origin/main 일치 확인 후 승인 범위만 명시적으로 커밋한다. 개발용 review 진입점은 Vite 프로덕션 빌드 대상이 아니며 outputs 캡처·보고서는 로컬 보존한다. GitHub Pages 업로드에서 public 아래 테스트 소스를 제외한다.
- 훈련일지 변경 모듈·preload·스타일은 `20260925-student-ux` 캐시 버전으로 통일한다. 공유 state.js는 단일 URL을 유지하고 새 워크스페이스 필드는 진입 시 초기화하여 기존 캐시와 호환한다.
- 실제 수강생 명단 조회 실패 시 예시 인원으로 보강 여석을 계산하지 않고, 오류 안내와 신청 확정 차단을 적용한다. 정상 수강생 정책·코치 표시·기존 데이터 저장 방식을 유지한다.
- 최종 검증: 전체64파일/562테스트·빌드·명단 실패 회귀11테스트·390px 예시 보강 확정과 상태 표시 확인. 배포 및 승인 공지 게시의 최종 결과·커밋은 공유 위키 `pages/geunryeok-school-app.md`에 기록한다.

### 성장 정보·이달의 PR 첫 화면 이동 (2026-09-25 로컬 후속)

- 사용자 요청으로 큰 게시판 환영 카드를 내 수업 최상단의 작은 성장 카드로 옮겼다. 이름·티어·학년·진행률·다음 학년까지 훈련량과 성장 그래프 진입을 제공한다. 로그아웃은 기존 내 정보에 유지한다.
- 상단 순서는 성장 정보 → 고정 공지 한 줄 → 이달의 PR → 내 수업/일정이다. 수강생 게시판의 PR 카드는 제거하고 코치 게시판의 위치는 유지한다. 공용 `MonthlyPRBanner`는 기존 최근30일 갱신자·티어·랭킹 진입과 한 명씩 순환하는 한 줄 표시를 제공한다.
- 티어·학년 통합 알림의 확인/미확인 상태는 `useStudentGrowth`·`studentGrowthState`·`studentGrowthService`가 관리한다. 계정 전환, 중복 조회, 확인 중 월 변경, 훈련일지 복귀를 처리한다. 확인 실패는 재시도하며 계산 실패를 XP 0으로 대체하지 않는다.
- 훈련일지 변경 엔트리/모듈/스타일 캐시는 `20260925-growth-alerts`, 공유 state URL은 유지한다. 새 Tailwind 클래스는 없다.
- 이 후속은 로컬 검증 후 사용자가 2026-09-25 배포를 승인했다. 이어 앞으로 이 프로젝트의 수정·검증 후 자동 커밋·푸시로 진행하기로 결정했다. 검증/캡처 정본은 `outputs/student-ux-local-2026-09-25/review.md`의 성장 정보 후속 절, 실제 배포 결과는 공유 위키에 기록한다.

- 조회량 최종 보완: 첫 화면 PR은 `monthlyPRPreviewService`에서 최근12건+작성자최대12명만 조회하고5분 메모리/sessionStorage 캐시·동시 요청 합류. 순환·다음·5분 내 재진입은 추가 조회0. 전체users/최근PR 무제한 스캔을 첫 화면에 추가하지 않는다.
- 고정 공지는 서버 `pinned==true` 조회·60초 캐시, 본인 읽음은1.5초 중복 억제. 실제 확인 후 캐시 최대revision을 병합해 N 재등장을 막는다. 고정 공지를 임의로 개수 제한하지 않는다.
- 성장 조회는 같은 월·성별 첫 성공 계산을 재사용하고 복귀3종 이벤트를1.5초로 합친다. 정상 첫4~5회/메뉴이동0/복귀1/확인1 본인문서 읽기. 월 records 범위조회 실패 시 과거전체로 확대하지 않는다. 기존 XP 최초시딩·본인자율운동이력 예외는 로컬 검토 기록 참조.
- 훈련일지 PR/학년 팝업은 한 번에 하나씩 표시한다. 최종68파일/632테스트·빌드·390/320px·실데이터 읽기전용 검증 통과. 변경 로직 lint오류0, 기존 App 경고1 및 auth.js의 기존5오류는 별도 확인했다.

### 보강 시간표 교시 표시 (2026-09-25)

- 수강생 보강 시간표 왼쪽에 코치 시간표와 동일한 `PERIODS` 기준 교시명(3교시는 자율 병기)을 시작·종료 시간 위에 표시한다. 교시명·시작 시각·종료 시각을 세로 배치하며 기존 신청 동작과 시간표 열 너비는 유지한다.
- 보강 시간표 배경은 일반 수업 연한 파랑·자율운동 연한 노랑·수업 없음/휴일 회색으로 구분하고 범례를 표시한다. 마감/비활성 여부와 무관하게 시간대 유형의 색을 유지하며 본인 정규/보강은 파란 테두리와 기존 상태 칩으로 구분한다.
