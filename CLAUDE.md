...
| `studentMeta/schedules` | 이름→시트 D열 수업일정 맵 단일 문서 `{map:{이름:'월1수1'}, updatedAt}`. 메인 앱이 코치 진입 시 `syncStudentSchedules`로 덮어씀(같은 이름 여러 행이면 오늘이 수강 기간인 행 우선). 훈련일지 코치 화면이 '지금 수업' 명단을 계산하는 유일 경로 — 발행 전이면 명단이 비므로 코치가 메인 앱을 한 번 열어야 한다 |
