...
13. **시트 원격 호출은 반복문 안에 넣지 말 것 (이 저장소에서 가장 자주 재발한 성능 버그).** 왕복 1회가 프로덕션 실측 **≈0.73초**(콜드스타트 3.7초)라 루프에 들어가면 그대로 초 단위로 쌓인다. 페이로드는 15개 시트 전부 합쳐 89KB로 작아서 **병목은 언제나 데이터 양이 아니라 왕복 횟수**다.
    ...
    - **재등록 검색은 `findStudentAcrossSheets(name, { scanAll: true })`.** 기본 2단계(±2개월 윈도우 → 폴백)는 활성 등록이 있는 학생 조회용이다. 재등록 대상은 정의상 수강이 끝난 사람이라 폴백이 100% 발동해 왕복만 늘어난다(실측 2.6초 → 1.1초). 학생 로그인·대시보드 경로는 기본값을 유지할 것 — 거기선 윈도우 1단계로 끝난다.
