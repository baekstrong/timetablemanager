...
## 자동 커밋·푸시 (2026-09-25 사용자 결정)

- 이 프로젝트는 요청받은 수정과 필요한 검증을 완료하면 별도 재승인 없이 이번 작업 파일만 커밋·푸시한다. main 푸시에 따른 GitHub Pages 자동 배포까지 진행하고 결과를 확인한다.
- 과거의 로컬 확인 후 배포 승인 대기·자동 커밋 금지 기록은 이전 단계의 이력이다. 사용자가 특정 작업의 보류를 새로 요청한 경우에는 그 요청을 따른다.
- 다른 작업자의 변경, 비밀값, 로컬 검토 산출물은 섞지 않는다. 공지 게시·문자/푸시 발송·운영 데이터 수정의 권한까지 확대하는 결정은 아니다.
